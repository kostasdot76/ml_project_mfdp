...
from services.service import UnitOfWork, DepositTransactionService, CreatePredictionTask
...
async def create_prediction_front(
    request: Request,
    model_type: str = Form(...),
    data: str = Form(...),  # Получаем JSON-строку из скрытого поля
    email: str = Depends(authenticate_cookie),
    session=Depends(get_session),
):
    try:
        ...
        with UnitOfWork(session) as uow:
            ...
            logging.info(f" prediction_service = CreatePredictionTask(: {user}")
            prediction_service = CreatePredictionTask(
                user=user,
                uow=uow,
                data=prediction_request,
            )
            logging.info(f"Инициализация init_rabbitmq")
            prediction_service.init_rabbitmq()
            # time.sleep(5)  # Ожидание инициализации RabbitMQ
            logging.info(f"Отправляем заказ в очередь")
            if prediction_service.process():
